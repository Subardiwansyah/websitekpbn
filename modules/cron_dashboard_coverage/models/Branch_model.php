...
class Branch_model extends CI_model {
	...
	function insert_data_branch()
  {
							...
							, (
										...
										FROM
												...
												INNER JOIN bd_tap xt
														ON (xl.id_tap = xt.id_tap)
												INNER JOIN bc_cluster xc
														ON (xt.id_cluster = xc.id_cluster)
										WHERE (xc.id_branch = b.id_branch
		...
		if (!empty($rs))
		{
			...
			{
				$this->db->select('1');
				$this->db->from('ae_dashboard_coverage_branch');
				$this->db->where('tahun', $tahun_sekarang);
				$this->db->where('bulan', (int) $bulan_sekarang);
				$this->db->where('id_branch', $rs[$x]['id_branch']);
				...
				if ($rsx)
				{
					...
					$this->db->where('bulan', (int) $bulan_sekarang);
					$this->db->where('id_branch', $rs[$x]['id_branch']);
					$this->db->update('ae_dashboard_coverage_branch', $data_x);
					$this->check_trans_status('update ae_dashboard_coverage_branch failed');
				}
				else
				{
					$data_x = array(
					...
					$this->db->insert('ae_dashboard_coverage_branch', $data_x);
					$this->check_trans_status('insert ae_dashboard_coverage_branch failed');
				}
			}
		}
	}

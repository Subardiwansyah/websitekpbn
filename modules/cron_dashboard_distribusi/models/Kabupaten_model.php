...
class Kabupaten_model extends CI_model {
	...
	function insert_data_kabupaten()
  {
		...
		if (!empty($rs))
		{
			...
			{
				$this->db->select('1');
				$this->db->from('am_dashboard_distribusi_kabupaten');
				...
				if ($rsx)
				{
					...
					$this->db->where('bulan', (int) $bulan_sekarang);
					$this->db->where('id_kabupaten', $rs[$x]['id_kabupaten']);
					$this->db->update('am_dashboard_distribusi_kabupaten', $data_x);
					$this->check_trans_status('update am_dashboard_distribusi_kabupaten failed');
				}
				else
				{
					$data_x = array(
					...
					$this->db->insert('am_dashboard_distribusi_kabupaten', $data_x);
					$this->check_trans_status('insert am_dashboard_distribusi_kabupaten failed');
				}
			}
		}
	}

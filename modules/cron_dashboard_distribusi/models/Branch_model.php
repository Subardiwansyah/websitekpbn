...
class Branch_model extends CI_model {
	...
	function insert_data_branch()
  {
		...
		if (!empty($rs))
		{
			...
			{
				$this->db->select('1');
				$this->db->from('aj_dashboard_distribusi_branch');
				...
				if ($rsx)
				{
					...
					$this->db->where('bulan', (int) $bulan_sekarang);
					$this->db->where('id_branch', $rs[$x]['id_branch']);
					$this->db->update('aj_dashboard_distribusi_branch', $data_x);
					$this->check_trans_status('update aj_dashboard_distribusi_branch failed');
				}
				else
				{
					$data_x = array(
					...
					$this->db->insert('aj_dashboard_distribusi_branch', $data_x);
					$this->check_trans_status('insert aj_dashboard_distribusi_branch failed');
				}
			}
		}
	}

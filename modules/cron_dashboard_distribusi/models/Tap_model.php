...
class Tap_model extends CI_model {
	...
	function insert_data_tap()
  {
		...
		if (!empty($rs))
		{
			...
			{
				$this->db->select('1');
				$this->db->from('al_dashboard_distribusi_tap');
				...
				if ($rsx)
				{
					...
					$this->db->where('bulan', (int) $bulan_sekarang);
					$this->db->where('id_tap', $rs[$x]['id_tap']);
					$this->db->update('al_dashboard_distribusi_tap', $data_x);
					$this->check_trans_status('update al_dashboard_distribusi_tap failed');
				}
				else
				{
					$data_x = array(
					...
					$this->db->insert('al_dashboard_distribusi_tap', $data_x);
					$this->check_trans_status('insert al_dashboard_distribusi_tap failed');
				}
			}
		}
	}

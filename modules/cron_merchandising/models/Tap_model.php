		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('mi_merchandising_res_tap');
					...
					if ($rs_d)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_share', $id_jenis_share);
						$this->db->update('mi_merchandising_res_tap', $data_x);
						$this->check_trans_status('update mi_merchandising_res_tap failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('mi_merchandising_res_tap', $data_x);
						$this->check_trans_status('insert mi_merchandising_res_tap failed');
					}
				}
			}
		}

...
class Sales_model extends CI_model {
	...
	function insert_data_outlet()
  {
		...
		$this->db->select('created_by AS id_sales');
		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('mj_merchandising_res_sales');
					...
					if ($rs_d)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_share', $id_jenis_share);
						$this->db->update('mj_merchandising_res_sales', $data_x);
						$this->check_trans_status('update mj_merchandising_res_sales failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('mj_merchandising_res_sales', $data_x);
						$this->check_trans_status('insert mj_merchandising_res_sales failed');
					}
				}
			}
		}

		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('mh_merchandising_res_cluster');
					...
					if ($rs_d)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_share', $id_jenis_share);
						$this->db->update('mh_merchandising_res_cluster', $data_x);
						$this->check_trans_status('update mh_merchandising_res_cluster failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('mh_merchandising_res_cluster', $data_x);
						$this->check_trans_status('insert mh_merchandising_res_cluster failed');
					}
				}
			}
		}

		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('mg_merchandising_res_branch');
					...
					if ($rs_d)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_share', $id_jenis_share);
						$this->db->update('mg_merchandising_res_branch', $data_x);
						$this->check_trans_status('update mg_merchandising_res_branch failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('mg_merchandising_res_branch', $data_x);
						$this->check_trans_status('insert mg_merchandising_res_branch failed');
					}
				}
			}
		}
		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
					...
					// MENGAMBIL DATA MERCHANDISING LOKASI
					$this->db->select('
						xx.telkomsel
						, xx.isat
						, xx.xl

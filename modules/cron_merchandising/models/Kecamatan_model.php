...
class Kecamatan_model extends CI_model {
	...
	function insert_data_outlet()
  {
		...
		if (!empty($rs_a))
		{
			...
			{
				$id_kecamatan = $rs_a[$a]['id_kecamatan'] ? $rs_a[$a]['id_kecamatan'] : 0;
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
						...
						(
							...
							FROM
									...
									INNER JOIN cd_kelurahan xkl
											ON (l.id_kelurahan = xkl.id_kelurahan)
							WHERE (xkl.id_kecamatan = "'.$id_kecamatan.'"
					...
					$this->db->select('1');
					$this->db->from('ml_merchandising_res_kecamatan');
					...
					if ($rs_d)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_share', $id_jenis_share);
						$this->db->update('ml_merchandising_res_kecamatan', $data_x);
						$this->check_trans_status('update ml_merchandising_res_kecamatan failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('ml_merchandising_res_kecamatan', $data_x);
						$this->check_trans_status('insert ml_merchandising_res_kecamatan failed');
					}
				}
			}
		}

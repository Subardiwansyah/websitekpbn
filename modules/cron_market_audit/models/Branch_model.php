		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('oz_maket_audit_res_branch');
					...
					if ($rs_d)
					{
						...
						$this->db->where('id_branch', $id_branch);
						$this->db->where('id_jenis_share', $id_jenis_share);
						$this->db->update('oz_maket_audit_res_branch', $data_x);
						$this->check_trans_status('update oz_maket_audit_res_branch failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('oz_maket_audit_res_branch', $data_x);
						$this->check_trans_status('insert oz_maket_audit_res_branch failed');
					}
				}
			}
		}

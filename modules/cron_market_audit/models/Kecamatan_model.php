		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($arr_share); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('oz_maket_audit_res_kecamatan');
					...
					if ($rs_d)
					{
						...
						$this->db->where('id_kecamatan', $id_kecamatan);
						$this->db->where('id_jenis_share', $id_jenis_share);
						$this->db->update('oz_maket_audit_res_kecamatan', $data_x);
						$this->check_trans_status('update oz_maket_audit_res_kecamatan failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('oz_maket_audit_res_kecamatan', $data_x);
						$this->check_trans_status('insert oz_maket_audit_res_kecamatan failed');
					}
				}
			}
		}

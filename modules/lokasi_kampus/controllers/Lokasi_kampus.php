...
class Lokasi_kampus extends Base_Controller {

	function __construct()
	{
		parent::__construct($securePage=true);

		$this->modul_name = 'lokasi_kampus';
		$this->daftar_display = 'List Kampus';
		$this->form_display = 'Form Kampus';
		$this->modul_display = 'Kampus';
		$this->view_list = 'lokasi_kampus_list_view';
		$this->view_form = 'lokasi_kampus_form_view';
		$this->view_view = 'lokasi_kampus_view_view';
		$this->view_download = 'lokasi_kampus_download_view';
		$this->load->model('Lokasi_kampus_model', 'data_model');
	}
		...
		foreach ($list as $field)
		{
			...
			if ($id_level == 4)
			{
				$row[] = '<center><div style="width:40px;">
					<button onClick="lihat(\''.$field->id_universitas.'\')" type="button" class="btn btn-primary btn-sm btn-icon waves-effect waves-themed" title="Lihat">
			...
			else
			{
				$row[] = '<center><div style="width:85px;">
					...
					<button onClick="ubah(\''.$field->id_universitas.'\')" type="button" class="btn btn-success btn-sm btn-icon waves-effect waves-themed" title="Ubah">
						<i class="fal fa-edit"></i>
					</button>
					<button onClick="hapus(\''.$field->id_universitas.'\')" type="button" class="btn btn-danger btn-sm btn-icon waves-effect waves-themed" title="Hapus">
		...
		foreach ($list as $field)
		{
			...
			else
			{
				$row[] = '<center><div style="width:85px;">
					...
					<button onClick="approved(\''.$field->id_universitas.'\')" type="button" class="btn btn-success btn-sm btn-icon waves-effect waves-themed" title="Approved">
						<i class="fal fa-check-square"></i>
					</button>
					<button onClick="rejected(\''.$field->id_universitas.'\')" type="button" class="btn btn-danger btn-sm btn-icon waves-effect waves-themed" title="Rejected">
	...
	function get_daftar_3()
	{
		...
		foreach ($list as $field)
		{
			...
			{
				$row[] = '<center><div style="width:40px;">
					<button onClick="lihat(\''.$field->id_universitas.'\')" type="button" class="btn btn-success btn-sm btn-icon waves-effect waves-themed" title="Lihat">
						<i class="fal fa-eye"></i>
					</button>
				</div></center>';
			}
	...
	function get_data_merchandising()
  {
	...
	function approved($id=0)
	{
		...
		$data['id_universitas'] = $this->uri->segment(3) ? $this->uri->segment(3) : 0;

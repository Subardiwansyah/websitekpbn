...
class Lokasi_outlet extends Base_Controller {

	function __construct()
	{
		parent::__construct($securePage=true);

		$this->modul_name = 'lokasi_outlet';
		$this->daftar_display = 'List Outlet';
		$this->form_display = 'Form Outlet';
		$this->modul_display = 'Outlet';
		$this->view_list = 'lokasi_outlet_list_view';
		$this->view_form = 'lokasi_outlet_form_view';
		$this->view_view = 'lokasi_outlet_view_view';
		$this->view_map = 'lokasi_outlet_map_view';
		$this->view_download = 'lokasi_outlet_download_view';
		$this->load->model('Lokasi_outlet_model', 'data_model');
	}
		...
		foreach ($list as $field)
		{
			...
			if ($id_level == 4)
			{
				$row[] = '<center><div style="width:40px;">
					<button onClick="lihat(\''.$field->id_outlet.'\')" type="button" class="btn btn-primary btn-sm btn-icon waves-effect waves-themed" title="Lihat">
			...
			else
			{
				$row[] = '<center><div style="width:85px;">
					...
					<button onClick="ubah(\''.$field->id_outlet.'\')" type="button" class="btn btn-success btn-sm btn-icon waves-effect waves-themed" title="Ubah">
						<i class="fal fa-edit"></i>
					</button>
					<button onClick="hapus(\''.$field->id_outlet.'\')" type="button" class="btn btn-danger btn-sm btn-icon waves-effect waves-themed" title="Hapus">
		...
		foreach ($list as $field)
		{
			...
			else
			{
				$row[] = '<center><div style="width:85px;">
					...
					<button onClick="approved(\''.$field->id_outlet.'\')" type="button" class="btn btn-success btn-sm btn-icon waves-effect waves-themed" title="Approved">
						<i class="fal fa-check-square"></i>
					</button>
					<button onClick="rejected(\''.$field->id_outlet.'\')" type="button" class="btn btn-danger btn-sm btn-icon waves-effect waves-themed" title="Rejected">
	...
	function get_daftar_3()
	{
		...
		foreach ($list as $field)
		{
			...
			{
				$row[] = '<center><div style="width:40px;">
					<button onClick="lihat(\''.$field->id_outlet.'\')" type="button" class="btn btn-success btn-sm btn-icon waves-effect waves-themed" title="Lihat">
						<i class="fal fa-eye"></i>
					</button>
				</div></center>';
			}
	...
	function lihat($id=0)
	{
		...
		$id = $this->uri->segment(3) ? $this->uri->segment(3) : 0;
		$lihat = $this->uri->segment(4) ? $this->uri->segment(4) : 0;
	...
	function get_data_merchandising()
  {
	...
	function validasi_form()
	{
		...
		$this->form_validation->set_rules('id_digipos', 'ID Digipos', 'required|trim|callback__cek_duplikasi');
	...
	function approved($id=0)
	{
		...
		$data['id_outlet'] = $this->uri->segment(3) ? $this->uri->segment(3) : 0;

		$this->load->view('approved_form_view', $data);
	}

	function validasi_approved()
	{
		$this->form_validation->set_rules('id_digipos', 'ID Digipos', 'callback__cek_duplikasi');

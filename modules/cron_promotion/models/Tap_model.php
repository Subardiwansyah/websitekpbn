		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($rs_b); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('nj_promotion_res_tap');
					...
					if ($rs_e)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_weekly', $id_jenis_weekly);
						$this->db->update('nj_promotion_res_tap', $data_x);
						$this->check_trans_status('update nj_promotion_res_tap failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('nj_promotion_res_tap', $data_x);
						$this->check_trans_status('insert nj_promotion_res_tap failed');
					}
				}
			}
		}

		...
		if (!empty($rs_a))
		{
			for ($a=0; $a<count($rs_a); $a++)
			{
				...
				for ($b=0; $b<count($rs_b); $b++)
				{
					...
					$this->db->select('1');
					$this->db->from('ni_promotion_res_cluster');
					...
					if ($rs_e)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_weekly', $id_jenis_weekly);
						$this->db->update('ni_promotion_res_cluster', $data_x);
						$this->check_trans_status('update ni_promotion_res_cluster failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('ni_promotion_res_cluster', $data_x);
						$this->check_trans_status('insert ni_promotion_res_cluster failed');
					}
				}
			}
		}

				...
				for ($b=0; $b<count($rs_b); $b++)
				{
					...
					$this->db->from('
						(
							...
							WHERE (kl.id_kecamatan = "'.$id_kecamatan.'"
					...
					$this->db->select('1');
					$this->db->from('nm_promotion_res_kecamatan');
					...
					if ($rs_e)
					{
						...
						$this->db->where('id_jenis_lokasi', $id_jenis_lokasi);
						$this->db->where('id_jenis_weekly', $id_jenis_weekly);
						$this->db->update('nm_promotion_res_kecamatan', $data_x);
						$this->check_trans_status('update nm_promotion_res_kecamatan failed');
					}
					else
					{
						$data_x = array(
						...
						$this->db->insert('nm_promotion_res_kecamatan', $data_x);
						$this->check_trans_status('insert nm_promotion_res_kecamatan failed');
					}
				}
			}
		}

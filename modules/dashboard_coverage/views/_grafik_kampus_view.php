				...
				<script>
					...
					var reload_grafik_kampus_1 = function(){
						Highcharts.chart('grafik_kampus_1', {
							...
							title: {
								text: '<span style="font-size:11px;">GRAFIK KAMPUS OPEN, CLOSE, NEW</span>'
							},
							xAxis: {
										...
										for($i=1; $i<=12; $i++)
										{
											...
											if($kategori == 'Branch')
											{
												...
												else
												{
													$query = $this->db->query('
															SELECT
																	COALESCE(SUM(d.kampus_open), 0) AS jumlah
															FROM
										...
										for($i=1; $i<=12; $i++)
										{
											...
											if($kategori == 'Branch')
											{
												...
												else
												{
													$query = $this->db->query('
															SELECT
																	COALESCE(SUM(d.kampus_close), 0) AS jumlah
															FROM
																	ae_dashboard_coverage_branch d
															WHERE d.tahun <= "'.$tahun.'"
																	AND d.bulan <= "'.$i.'"
										...
										for($i=1; $i<=12; $i++)
										{
											...
											if($kategori == 'Branch')
											{
												...
												else
												{
													$query = $this->db->query('
															SELECT
																	COALESCE(SUM(d.kampus_new), 0) AS jumlah
															FROM
																	ae_dashboard_coverage_branch d
															WHERE d.tahun <= "'.$tahun.'"
																	AND d.bulan <= "'.$i.'"
									...
									<?php
										for($i=1; $i<=12; $i++)
										{
											...
											if($kategori == 'Branch')
											{
												...
												$query = $this->db->query('
														SELECT
																COALESCE(SUM(d.kampus_pjp), 0) AS jumlah
														FROM
																ae_dashboard_coverage_branch d
														WHERE d.tahun = "'.$tahun.'"
																AND d.bulan = "'.$i.'"
									...
									<?php
										for($i=1; $i<=12; $i++)
										{
											...
											if($kategori == 'Branch')
											{
												...
												$query = $this->db->query('
														SELECT
																COALESCE(SUM(d.kampus_clockin), 0) AS jumlah
														FROM
																ae_dashboard_coverage_branch d
														WHERE d.tahun = "'.$tahun.'"
																AND d.bulan = "'.$i.'"
									...
									<?php
										for($i=1; $i<=12; $i++)
										{
											...
											if($kategori == 'Branch')
											{
												...
												$query = $this->db->query('
														SELECT
																COALESCE(SUM(d.kampus_coverage), 0) AS jumlah
														FROM
																ae_dashboard_coverage_branch d
														WHERE d.tahun = "'.$tahun.'"
																AND d.bulan = "'.$i.'"

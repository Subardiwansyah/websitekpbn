...
class Cluster_model extends CI_model {
	...
	function insert_data_cluster()
  {
		...
		if (!empty($rs))
		{
			...
			{
				$this->db->select('1');
				$this->db->from('af_dashboard_coverage_cluster');
				...
				if ($rsx)
				{
					...
					$this->db->where('bulan', (int) $bulan_sekarang);
					$this->db->where('id_cluster', $rs[$x]['id_cluster']);
					$this->db->update('af_dashboard_coverage_cluster', $data_x);
					$this->check_trans_status('update af_dashboard_coverage_cluster failed');
				}
				else
				{
					$data_x = array(
					...
					$this->db->insert('af_dashboard_coverage_cluster', $data_x);
					$this->check_trans_status('insert af_dashboard_coverage_cluster failed');
				}
			}
		}
	}
